...
var chratoscoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in chratoscoreDefaults)
    if (chratoscoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = chratoscoreDefaults[i];
