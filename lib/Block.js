...
Block.prototype.checkBlock = function checkBlock(txs) {
  ...
  //if (this.prototype.height > BlockRules.lastPOWBlock)
...
Block.getBlockValue = function getBlockValue(height) {
  var subsidy = 0;
  if (height == 0 )
    subsidy = 100 * util.COIN;
  if (height > 0 && height <= 10 )
    subsidy = 550 * util.COIN;
  if (height > 10 && height < 2000) {
    subsidy = 500 * util.COIN;
  }
  // no halving for PoW
...
Block.prototype.createCoinstakeTx = function createCoinstakeTx(beneficiary) {
  var tx = new Transaction();
  return tx;
};
